...
class VecoDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some vecod methods
    ...
    @classmethod
    def from_veco_conf(self, veco_dot_conf):
        from veco_config import VecoConfig
        config_text = VecoConfig.slurp_config_file(veco_dot_conf)
        creds = VecoConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = vecolib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = vecolib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = vecolib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_vecod_proto_version_with_sentinel_ping)
