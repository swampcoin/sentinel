...
def has_veco_conf():
    ...
    valid_veco_conf = False

    # ensure veco_conf exists & readable
    #
    # if not, print a message stating that Veco Core must be installed and
    # configured, including JSONRPC access in veco.conf
    try:
        f = io.open(config.veco_conf)
        valid_veco_conf = True
    except IOError as e:
        print(e)

    return valid_veco_conf
...
def main():
    ...
    if not has_veco_conf():
        print("VecoCore must be installed and configured, including JSONRPC access in veco.conf")
        sys.exit(1)
