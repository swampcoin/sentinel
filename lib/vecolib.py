...
def is_valid_veco_address(address, network='mainnet'):
    ...
    veco_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Veco address
        return False

    if (address_version != veco_version):
        return False
...
# shims 'til we can fix the vecod side
def SHIM_serialise_for_vecod(sentinel_hex):
    ...
    # superblock => "trigger" in vecod
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # vecod expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    vecod_hex = serialise(obj)
    return vecod_hex


# shims 'til we can fix the vecod side
def SHIM_deserialise_from_vecod(vecod_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(vecod_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('veco.conf')
