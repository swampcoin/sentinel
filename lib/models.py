...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, vecod):
        golist = vecod.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_vecod(vecod, item)
    ...
    @classmethod
    def import_gobject_from_vecod(self, vecod, rec):
        import decimal
        import vecolib
        import inflection
        ...
        # shim/vecod conversion
        object_hex = vecolib.SHIM_deserialise_from_vecod(object_hex)
        objects = vecolib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from vecod...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from vecod, with every run
        ...
        # get/create, then sync payment amounts, etc. from vecod - Vecod is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(vecod)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from vecod! %s" % e)
            govobj.vote_delete(vecod)
            return (govobj, None)
    ...
    def vote_delete(self, vecod):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(vecod, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, vecod, signal, outcome):
        import vecolib
        ...
        output = vecod.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = vecolib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(vecod, signal)

    def sync_network_vote(self, vecod, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = vecod.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 veco addr, non-multisig
            if not vecolib.is_valid_veco_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Veco address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (vecolib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/VecoDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import vecolib
        obj_data = vecolib.SHIM_serialise_for_vecod(self.serialise())
    ...
    def prepare(self, vecod):
        try:
            object_hash = vecod.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not vecolib.is_valid_veco_address(addr, config.network):
    ...
    def hash(self):
        import vecolib
        return vecolib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, vecod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - vecod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, vecod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - vecod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, vecod):
        now = int(time.time())
        return (self.created_at < (now - vecod.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, vecod):
        if self.is_expired(vecod):
            return False

        return True

    def is_deletable(self, vecod):
        if self.is_expired(vecod):
            return True
