...
def test_valid_veco_address():
    from vecolib import is_valid_veco_address

    main = valid_veco_address()
    test = valid_veco_address('testnet')

    assert is_valid_veco_address(main) is True
    assert is_valid_veco_address(main, 'mainnet') is True
    assert is_valid_veco_address(main, 'testnet') is False

    assert is_valid_veco_address(test) is False
    assert is_valid_veco_address(test, 'mainnet') is False
    assert is_valid_veco_address(test, 'testnet') is True


def test_invalid_veco_address():
    from vecolib import is_valid_veco_address

    main = invalid_veco_address()
    test = invalid_veco_address('testnet')

    assert is_valid_veco_address(main) is False
    assert is_valid_veco_address(main, 'mainnet') is False
    assert is_valid_veco_address(main, 'testnet') is False

    assert is_valid_veco_address(test) is False
    assert is_valid_veco_address(test, 'mainnet') is False
    assert is_valid_veco_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from vecolib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % vecolib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(vecolib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(vecolib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(vecolib.blocks_to_seconds(16616)) == 2612035
